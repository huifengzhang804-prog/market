...
public class TLSSigAPIv2 {
    ...
    private String genUserSig(String userid, long expire, byte[] userbuf) {
        ...
        try {
            ...
            compressor.finish();
            compressedBytes = new byte[2048];
            compressedBytesLength = compressor.deflate(compressedBytes);
            compressor.end();
        } catch (Exception e) {
            log.error("鉴权票据生成失败", e);
            throw new GlobalException(SystemCode.REQ_REJECT_CODE, "鉴权票据生成失败");
        }
        return Base64.getEncoder().encodeToString(Arrays.copyOfRange(compressedBytes,
