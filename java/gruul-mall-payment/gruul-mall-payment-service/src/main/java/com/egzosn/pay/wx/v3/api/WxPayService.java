...
//        wxParameterStructure.getPublicParameters(parameters);
    ...
    public HttpStringEntity hookHttpEntity(HttpStringEntity entity) {
        String keyPublicId = payConfigStorage.getKeyPublicId();
        String serialNo = StringUtils.isNotEmpty(keyPublicId) ? keyPublicId : payConfigStorage.getCertEnvironment().getPlatformSerialNumber();
        entity.addHeader(new BasicHeader(WxConst.WECHATPAY_SERIAL, serialNo));
        return entity;
    }
