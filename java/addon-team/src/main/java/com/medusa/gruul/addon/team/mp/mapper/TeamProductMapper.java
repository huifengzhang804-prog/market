...
public interface TeamProductMapper extends BaseMapper<TeamProduct> {
     ...
     * 获取拼团商品统计数据
     *
     * @param keys 拼团商品key集合
     * @return 拼团商品统计数据
     */
    List<TeamProductCount> teamProductCounts(@Param("keys") Set<TeamKey> keys);
     ...
     * @return 分页结果数据
     */
    IPage<TeamActivityProductVO> activityProductPage(@Param("query") Page<TeamActivityProductVO> query);
     ...
     * @return 参与人数
     */
    Set<TeamProductUserCount> teamProductUserCounts(@Param("keys") Set<ActivityShopProductKey> keys);
     ...
     * @return 商品名称和商品图片
     */
    Set<TeamProductInfoCount> productsInfo(@Param("keys") Set<ActivityShopProductKey> keys);
     ...
     */
    List<TeamProduct> getCurrentTeamProduct(@Param("skuList") List<StorageSku> skuList);
}
