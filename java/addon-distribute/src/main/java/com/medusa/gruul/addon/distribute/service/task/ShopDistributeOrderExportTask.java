...
import com.medusa.gruul.addon.distribute.model.dto.ShopDistributeOrderExportDTO;
 ...
 * @since 2025/2/17
 */
public class ShopDistributeOrderExportTask extends AbstractDistributeOrderExportTask {


    public static final String SHOP_DISTRIBUTE_ORDER_FILE_PATH = "/temp/商家端--分销订单.xlsx";

    public ShopDistributeOrderExportTask(
    ...
    @Override
    public void run() {
        File tempExcelFile = generateTempExcelFile(SHOP_DISTRIBUTE_ORDER_FILE_PATH);
     ...
     * @return
     */
    public List<ShopDistributeOrderExportDTO> data(List<DistributorMainOrderVO> records, AtomicInteger index) {
        List<ShopDistributeOrderExportDTO> result = Lists.newArrayList();
    ...
    private void dealItem(DistributorMainOrderVO order,
                          ...
                          List<ShopDistributeOrderExportDTO> result,
                          AtomicInteger index, Map<Long, UserDataVO> userDataVOMap, Map<String, OrderInfoDTO> orderInfoDTOMap) {
        ShopDistributeOrderExportDTO row = new ShopDistributeOrderExportDTO();
        //编号
    ...
    ExcelWriter fetchExcelWriter(File excelFile) throws IOException {
        return EasyExcel.write(excelFile, ShopDistributeOrderExportDTO.class)
//                    .registerWriteHandler(new CellStyleStrategy())
                .withTemplate(new ClassPathResource(SHOP_DISTRIBUTE_ORDER_FILE_PATH).getInputStream())
