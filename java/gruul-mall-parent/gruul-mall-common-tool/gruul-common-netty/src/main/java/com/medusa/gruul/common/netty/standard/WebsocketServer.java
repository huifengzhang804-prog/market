...
import com.medusa.gruul.common.netty.standard.handler.HttpServerHandler;
import com.medusa.gruul.common.netty.util.SslUtils;
...
//        }));
        ...
        for (int port : webSocketEndpointConfig.getPorts()) {
            ...
            if ("0.0.0.0".equals(webSocketEndpointConfig.getHost())) {
                ...
                try {
                    channelFuture = bootstrap.bind(new InetSocketAddress(InetAddress.getByName(webSocketEndpointConfig.getHost()), port));
                } catch (UnknownHostException e) {
                    channelFuture = bootstrap.bind(webSocketEndpointConfig.getHost(), port);
                    e.printStackTrace();
                }
            }
