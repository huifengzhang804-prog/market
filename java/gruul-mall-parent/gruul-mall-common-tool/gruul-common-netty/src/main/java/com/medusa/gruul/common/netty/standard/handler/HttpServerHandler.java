package com.medusa.gruul.common.netty.standard.handler;
...
public class HttpServerHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    private void handleHttpRequest(ChannelHandlerContext ctx, FullHttpRequest req) {
        ...
        if (!ObjectUtils.isEmpty(webSocketEndpointServer.getHost()) && !"0.0.0.0".equals(webSocketEndpointServer.getHost()) && !webSocketEndpointServer.getHost().equals(host.split(":")[0])) {
            if (forbiddenByteBuf != null) {
                res = new DefaultFullHttpResponse(HTTP_1_1, FORBIDDEN, forbiddenByteBuf.retainedDuplicate());
            } else {
                res = new DefaultFullHttpResponse(HTTP_1_1, FORBIDDEN);
