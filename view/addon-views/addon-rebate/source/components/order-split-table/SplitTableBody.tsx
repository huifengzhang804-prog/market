...
import { RebateOrderList } from '../types'

const buildProps = () => ({
  data: {
    type: Array as PropType<RebateOrderList[]>,
    default() {
      return []
    },
  },
...
export default defineComponent({
  ...
  setup(prop, { slots, emit }) {
    const records = ref<RebateOrderList[]>([])
     ...
     * @param orders
     */
    const itemsToPackageMap = (orders: RebateOrderList[]) => {
      if (!orders) return []
      orders.forEach((order) => {
        const shopOrder = order
        const shopOrderItems = shopOrder.rebateOrderItems
        shopOrder.packageMap = shopOrderItems.reduce((map, currentItem) => {
          ...
          return map
        }, new Map<string | undefined, RebateOrderList['rebateOrderItems'][]>())
      })
      return orders
    }
    watch(
    ...
    const packageMapToDom = (row: RebateOrderList, index: number): any[] => {
      const tds: any[] = []
      const packageMap = row.packageMap
