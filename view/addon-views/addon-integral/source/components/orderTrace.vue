...
import Teps from '@/components/teps.vue'
import 'uno.css'
...
<template>
  ...
  <div class="goods-container">
    <div style="padding: 0 20px 20px">商品信息</div>
      ...
      <div v-else>
        <div class="logistics-container__label">
          快递公司：<span class="logistics-container__value">
            {{ props?.integralOrder?.expressName }}
          </span>
        </div>
        <div class="logistics-container__label">
          快递单号：
          <div class="logistics-container__no">
            <span class="logistics-container__value">{{ props?.integralOrder?.expressNo }}</span>
          </div>
        </div>
  ...
  <!-- 物流信息 e -->
  <div v-if="orderLocation[0].context">
    <Teps :list="orderLocation">
      <template #icon>
        <div style="width: 7px; height: 7px; border-radius: 50%; background-color: #a8abb2"></div>
      </template>
    </Teps>
  </div>
  <div v-else class="full-screen-empty" text="暂无物流信息" mode="address"></div>
</template>
<!-- 物流步骤条 -->
