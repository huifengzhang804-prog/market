...
import OrderTable from './distributionOrderTable.vue'
import PageManage from '@/components/PageManage.vue'
import { doExportDisOrder, doGetDisOrder } from '../apis'
import DateUtil from '@/utils/date'
import OrderIllustration from './orderIllustration.vue'
...
// 订单说明弹窗
const orderIllustrationShow = ref(false)
const Illustration = (val: boolean) => {
  orderIllustrationShow.value = val
}
const IllustrationFn = () => {
  orderIllustrationShow.value = true
}
...
<template>
  <el-config-provider :empty-values="[undefined, null]">
    <SchemaForms v-model="searchForm" :columns="columns" @searchHandle="handleSearch" @handleReset="handleReset">
  ...
  <div style="position: relative">
        ...
        <template #label>
          ...
          <el-dropdown placement="bottom-end" trigger="click" @command="handleQuickSearchCommand">
            <span class="el-dropdown-link" style="height: 40px" @click.stop="() => {}">
              <el-icon class="el-icon--right" style="position: relative; top: 13px">
                <ArrowDown />
              </el-icon>
            </span>
    ...
    <QIcon name="icon-wenhao" size="18px" color="#666" style="position: absolute; right: 16px; top: 12px; cursor: pointer" @click="IllustrationFn" />
  </div>
  <div class="distribution_order_table_container" @scroll="onScroll">
    <div class="tbhead">
      <el-checkbox v-model="isAllSelected" style="margin-left: 16px" :indeterminate="isIndeterminate" @change="handleSelectAll"></el-checkbox>
  ...
  />
  <OrderIllustration v-model:order-illustration-show="orderIllustrationShow" @illustration="Illustration"></OrderIllustration>
</template>
