...
import { ref, defineProps, defineEmits, provide, computed, watch, onMounted } from 'vue'
import { useVModel } from '@vueuse/core'
import FreightTemplateForm from './freight-template-form.vue'
import uuid from '@/utils/uuid'
import { ElMessage, ElMessageBox } from 'element-plus'
import { doAddLogisticsTemplate, doGetLogisticsTemplateInfoById, doPutLogisticsTemplateInfoById } from '../apis'
...
const addCloneList = ref<FreightTemplateFormSubmitData>()
const editCloneList = ref<FreightTemplateFormSubmitData>()
onMounted(() => {
  addCloneList.value = Object.assign({}, freightTemplateFormSubmitData.value)
})
...
async function initTemplateEditInfo() {
  if ($props.id) {
    ...
    editCloneList.value = Object.assign({}, freightTemplateFormSubmitData.value)
  }
}
const freightTemplateFormRef = ref()

const handleClose = () => {
  if ($props.id && JSON.stringify(editCloneList.value) === JSON.stringify(freightTemplateFormRef.value.freightTemplateForm)) {
    removeData()
    _isShow.value = false
  } else if (!$props.id && JSON.stringify(addCloneList.value) === JSON.stringify(freightTemplateFormSubmitData.value)) {
    removeData()
    _isShow.value = false
  } else {
    ElMessageBox.confirm('关闭后内容将不保留?', '提示', {
...
async function handleEditTemplateSubmit() {
  ...
  !choiceConditionPostage && (freightTemplateFormSubmitData.value.logisticsIncludePostageDTO = undefined)
...
function submitDataValidate() {
  ...
  if (choiceConditionPostage && !logisticsIncludePostageDTO.every((item: any) => item.region.length)) {
  ...
  if (isQuantityBeGreaterThanOne) {
    valuationModel === 'PKGS' ? ElMessage.error('件数最少1') : ElMessage.error('重量最少0.1')
    return false
  }
  // 首费用
...
function handleSubmitDataDelId() {
  ...
  if (choiceConditionPostage) {
    freightTemplateFormSubmitData.value.logisticsIncludePostageDTO = logisticsIncludePostageDTO.map((item: any) => {
      item.id = undefined
      return item
    })
    return
...
<template>
  <el-dialog v-model="_isShow" :before-close="handleClose">
    <template #header="{ titleId, titleClass }">
      <div class="my-header">
        <h4 :id="titleId" :class="titleClass">{{ `${$props.id ? '编辑' : '添加'}` }}运费模板</h4>
      </div>
    </template>
    <FreightTemplateForm ref="freightTemplateFormRef" :isEdit="!!$props.id" :isPKGS="isPKGS" />
    <template #footer>
