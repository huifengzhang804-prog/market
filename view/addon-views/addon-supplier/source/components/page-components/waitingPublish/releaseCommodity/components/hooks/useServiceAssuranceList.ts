...
const useServiceAssuranceList = (submitForm: Ref<SubmitFormInterface>) => {
  const serviceAssuranceList = ref(basicServiceAssuranceList)
  function updateserviceIds() {
    submitForm.value.serviceIds = serviceAssuranceList.value
      .filter((item) => item.state)
      .map((item) => item.enum) as SubmitFormInterface['serviceIds']
  }
  function initServiceAssuranceList() {
    if (submitForm.value.serviceIds.length) {
      serviceAssuranceList.value = serviceAssuranceList.value.map((item) => {
  ...
  onActivated(() => initServiceAssuranceList())
