...
const $props = defineProps({
  currentProduct: {
    type: Object as PropType<ReleaseList>,
    default: () => ({}),
  },
})
...
const getProductDetails = async () => {
  // 请求关联的SKU信息用于数据展示
  const { data } = await doGetCommoditySku(useShopInfoStore().shopInfo.id, $props.currentProduct.id)
...
<template>
  <div class="good">
    ...
    <div class="good__img">
      <text>商品图片：</text>
      <el-image :preview-src-list="[$props.currentProduct.pic]" :src="$props.currentProduct.pic" style="width: 100px; height: 100px"></el-image>
    </div>
    <div>规格：</div>
    <el-table :data="skuList" :header-row-style="{ 'font-size': '12px', color: '#000000' }" height="350" stripe>
      <el-table-column v-if="specGroups.length" align="center" label="规格">
        <template #default="{ row }">
          <span>{{ row?.specs && row?.specs.join('-') }}</span>
        </template>
      </el-table-column>
      <el-table-column v-if="skuList && skuList[0]?.image" align="center" label="sku图">
        <template #default="{ row }">
