...
import { LimitTypeEnum, SkuInterface } from '../components/consignment-page/types/list'
import useDistribution from '../components/consignment-page/goods-source/hooks/useDistribution'
import BatchUnifySettings from '../components/consignment-page/goods-source/components/batch-unify-settings.vue'
...
<template>
  <div class="distribution">
    <el-form ref="formRef" class="distribution_form fdc1 overh" :model="distributionFormModel" :rules="distributionFormRules">
      ...
      <template v-if="distributionSkus.length === 1">
        ...
        <div class="distribution__container">
          <el-row :gutter="8">
            ...
            <!-- <el-col :span="6">供货售价：{{ divTenThousand(distributionSkus?.[0]?.price) }}</el-col> -->
          </el-row>
        </div>
      </template>
      ...
      <div class="distribution__container fdc1 overh">
        <div class="price-settings">
          <el-radio-group v-model="distributionFormModel.consignmentPriceSetting.type">
            <el-radio :disabled="distributionFormModel.unifyPriceSetting.type" value="UNIFY">统一设价</el-radio>
            <el-radio value="RATE">按比例设价</el-radio>
            <el-radio value="REGULAR">固定金额设价</el-radio>
          </el-radio-group>
          <template v-if="distributionFormModel.consignmentPriceSetting.type === 'UNIFY'">
            <BatchUnifySettings
              :sale="+distributionFormModel.unifyPriceSetting.sale"
              :scribe="+distributionFormModel.unifyPriceSetting.scribe"
              :type="distributionFormModel.unifyPriceSetting.type"
            />
          </template>
          <template v-else-if="distributionFormModel.consignmentPriceSetting.type === 'RATE'">
            <el-form-item prop="consignmentPriceSetting.sale">
              <span>销售价 = 供货价 + （供货价 * &nbsp;</span>
              <el-input-number v-model="distributionFormModel.consignmentPriceSetting.sale" :controls="false" :precision="2" :min="0" />
              <span>&nbsp;% ）</span>
            </el-form-item>
            <el-form-item prop="consignmentPriceSetting.scribe">
              <span>划线价 = 销售价 + （销售价 * &nbsp;</span>
              <el-input-number v-model="distributionFormModel.consignmentPriceSetting.scribe" :controls="false" :precision="2" :min="0" />
              <span>&nbsp;% ）</span>
            </el-form-item>
          </template>
          <template v-else>
        ...
        <el-table :data="distributionSkus" :max-height="500" class="f1">
          <el-table-column v-if="distributionSkus[0]?.image" fixed="left" width="150" label="规格图">
          ...
          <el-table-column width="150" label="规格限购">
            <template #default="{ row }: { row: SkuInterface }">
              {{ (row?.limitType !== 'UNLIMITED' && row?.limitNum) || '/' }}
            </template>
          </el-table-column>
    ...
    <div class="distribution__btns">
      ...
      <el-button style="margin-left: 40px" type="primary" @click="handleConfirmDistribution">确认</el-button>
    </div>
  </div>
</template>
