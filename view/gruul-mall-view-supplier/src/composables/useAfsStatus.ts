import { A_REfUND_WHY, ARefundType, AFSSTATUS, PACKAGESTATUS, ApiOrderAfsItem } from '@/views/afs/types'
import { calculate } from '@/views/order/orderDetails/OrderStatusCalculate'
import { ApiOrder, ORDERPAYMENT, ORDERSTATUS } from '@/views/order/types/order'
...
export { aRefundWhy, afsPagesStatus, TabItem, systemInfo, packageStatus, getAfsListStatusCn, getAfsNumStatusCn, getAfsOrderStatusCn, getAfsStatusCn }
...
/**
 * @description: 售后订单信息状态转中文
 * @param {keyof} status
 * @returns {*}
 */
export const getAfsOrderInfoStatusCn = (order: ApiOrder) => {
    return calculate(order).state.status
}
/**
 * @description: 售后列表售后状态转化中文
