...
export const configurePlatform = defineStore('configurePlatform', {
    ...
    getters: {
        ...
        // 获取供应商的登录页
        getSupplierLoginPageBg(state): string {
            return state.websiteForm.SUPPLIER_LOGIN_PAGE_BG
        },
        // 获取供应商的网站名称
        getSupplierWebSitName(state): string {
            return state.websiteForm.SUPPLIER_WEB_SIT_NAME
        },
        getSetNews(state): number {
            return state.NOTICE_NEWS
        },
    },
})
