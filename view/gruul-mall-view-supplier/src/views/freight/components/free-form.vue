...
const getAreaName = (data: ChooseAreaItem[]) => {
    return data
        .map((item) => {
            ...
            return `${item.upperName}(${item.lowerName && item.lowerName.length ? item.lowerName.length : ''}/${
                item && item.length ? item.length : ''
            })`
        })
        .join(',')
}
        ...
        <el-table-column label="操作" prop="name" align="center" width="120" fixed="right">
            <template #default="{ row, $index }">
                <div class="dialogTab_container__right_btn">
                    ...
                    <el-button :disabled="_tableData.length === 1" link type="danger" @click="HandleSecondTabItem(_tableData.length - 1, row)"
                        >删除</el-button
                    >
                </div>
            </template>
