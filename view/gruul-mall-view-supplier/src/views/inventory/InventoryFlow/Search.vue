<template>
    <SchemaForm v-model="searchType" :columns="columns" @searchHandle="search" @handleReset="handleReset">
        <template #otherOperations>
            <el-button type="primary" @click="$emit('onExportData', searchType)">导出</el-button>
        </template>
    </SchemaForm>
</template>
...
export type SearchType = Record<'productName' | 'productId' | 'stockChangeType' | 'sellType' | 'orderNo' | 'date' | 'Id', string>
...
const $emit = defineEmits(['onSearchParams', 'onExportData'])
