<template>
    ...
    <div class="table_container">
        ...
        >
            ...
            <el-table-column prop="dataCount" label="数据量(条)" width="120" />
            <el-table-column prop="fileSize" label="文件大小" width="120">
                <template #default="{ row }">
                    {{ getFileSize(row?.fileSize) }}
                </template>
            </el-table-column>
            <el-table-column prop="userPhone" label="导出用户" width="120" />
            <el-table-column prop="status" label="状态">
                <template #default="{ row }">
                    <span :style="{ color: row?.status === 'SUCCESS' ? 'rgb(2,187,78)' : row?.status === 'FAILED' ? '#f56c6c' : '' }">
...
const initialData = async () => {
    let centerData: any[] = [],
        total = 0
    try {
        const { data } = await doGetDownloadCenterList({
            ...searchOptions,
            current: paginationOptions.value.current,
            size: paginationOptions.value.size,
        })
        total = +data?.total
        centerData = data?.records || []
    } finally {
        paginationOptions.value.total = total
...
const handleSizeChange = (value: number) => {
    ...
    paginationOptions.value.size = value
    initialData()
}
const handleCurrentChange = (value: number) => {
    paginationOptions.value.current = value
    initialData()
}
</script>
