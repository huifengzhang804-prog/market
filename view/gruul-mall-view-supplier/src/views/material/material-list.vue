    ...
    try {
        ...
        if (upload.value) {
            ...
            if (res.data.code === 200) {
                ...
                new Storage().setItem('supplyParentId', upload.value.categoryFormModel.parentId)
...
const handleSelect = (row: Obj) => {
    if (checkedSelect.value.find((item) => item.id === row.id)) {
        checkedSelect.value = checkedSelect.value.filter((item) => item.id !== row.id)
    } else {
        ...
        <div class="sourceMaterial_container">
            ...
            >
                <video
                    ...
                    :class="{
                        isChecked: checkedSelect.find((item) => item.id === row.id),
                    }"
                    autoplay
                    controls
                    loop
                ...
                <q-icon v-show="checkedSelect.find((item) => item.id === row.id)" name="icon-dui" class="checked_icon"></q-icon>
