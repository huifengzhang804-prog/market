...
<template>
    ...
    <div class="table_container">
        <el-table
            ...
            :cell-style="{ fontSize: '14px', color: '#333333', height: '70px' }"
        >
            ...
            <el-table-column label="二级类目" prop="currentCategoryName">
                <template #default="{ row }">
                    <div style="display: flex; align-items: center">
                        <image :src="row.imageUrl" alt="" />
                        <el-image style="width: 50px; height: 50px; margin-right: 3px" :src="row.imageUrl" fit="fill" />
                        <span>{{ row.currentCategoryName }}</span>
                    </div>
                </template>
            </el-table-column>
            ...
            <el-table-column label="操作" width="100px" align="right">
                <template #default="{ row }">
                    <el-button link type="danger" @click="handleRemoveClick(row.id)">移除</el-button>
                </template>
            </el-table-column>
        </el-table>
    </div>
