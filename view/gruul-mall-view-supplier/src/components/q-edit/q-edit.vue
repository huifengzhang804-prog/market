...
let cacheImgPromiseList: Promise<string>[] = []
    ...
    MENU_CONF: {
        ...
        uploadImage: {
            ...
            customUpload(file: File, insertFn: InsertFnType) {
                cacheImgPromiseList.push(uploadFile('gruul-mall-carrier-pigeon/oss/upload', file))
                setTimeout(() => {
                    Promise.all(cacheImgPromiseList).then((res) => {
                        res.forEach((item) => {
                            insertFn(item, '', '')
                        })
                    })
