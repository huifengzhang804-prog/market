...
import type { CascaderProps, UploadRawFile } from 'element-plus'
...
const beforeAvatarUpload: UploadProps['beforeUpload'] = (rawFile) => {
    ...
    if (!rawFileType.includes(rawFile.type)) {
        ...
        // ElMessage.error('上传素材类型必须是 jpeg、jpg、png、gif、mp4 其中之一')
        return false
...
const handleRemove: UploadProps['onRemove'] = (uploadFile, uploadFiles) => {
    ...
    else fileLists.value = uploadFiles.map((file) => file.raw) as UploadRawFile[]
}
const handlePictureCardPreview: UploadProps['onPreview'] = (uploadFile) => {
    dialogImageInfo.url = uploadFile.url!
    dialogImageInfo.isMedia = uploadFile?.raw?.type?.startsWith('video') as boolean
...
<template>
    <el-form ref="formRef" :model="categoryFormModel" :rules="formFules" label-width="95px" label-position="left">
        <el-form-item label="上传至分类" prop="parentId">
            ...
            <el-button type="primary" link style="margin-left: 20px" @click="($event: MouseEvent) => handleAddCategory()">创建分类</el-button>
            <span v-if="materialCategoryList?.length < 1" style="color: #999; margin-left: 10px">请创建分类后再上传</span>
        </el-form-item>
