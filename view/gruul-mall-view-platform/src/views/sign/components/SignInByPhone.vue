    ...
    <div v-loading="loading" class="SignFlow">
        ...
        >
            ...
            <el-form-item prop="account" style="margin-left: 0">
                <el-input v-model="signForm.account" :maxlength="11" placeholder="请输入手机号 或 账号">
                    <template #prepend>
...
import { signByUser, doPostSmsCode, doGetUserMenu, doGetAdminMine, doGetCaptchaSlider } from '@/apis/sign'
...
function submitForm(e: Event) {
    ...
    formRef.validate((valid: boolean) => {
        if (!valid) return
        signByUser(signHandler.value.grantType, signHandler.value.formData()).then(async ({ code, data, msg }) => {
            if (code !== 200) {
                ElMessage.error(msg)
                return
            }
            ...
            await router.push({
                path: redirect ? redirect.toString() : '/',
            })
        })
    })
}
...
async function getUserData() {
    const { data, code } = await doGetAdminMine()
    if (code === 200 && data) {
        new Storage().setItem('userInfo', data)
    }
}
/**
 * lifeCircle
