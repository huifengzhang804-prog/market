...
import { doGetAfssHistory, doGetUserDataByBuyerId } from '@/apis/afs'
import type { ProcessingHistory, AFSSTATUS } from '@/views/afs/types/'
import type { ApiBuyersData, ApiHistory } from '@/views/afs/types/api'
...
async function initBuyersData() {
    ...
    afsConfigStatus.value = getAfsOrderStatusCn(data[0].status)
...
async function initHistory() {
    const afsNo = String($route.query.afsNo)
    const { code, data } = await doGetAfssHistory<ApiHistory>(afsNo)
    newAfsTime.value = data[0].createTime
    if (code !== 200) {
...
<template>
    <div class="afterSalesInfo">
        <div class="afterSalesInfo__status">
            <div class="afterSalesInfo__status--title">售后状态：{{ $route.query.statusContent }}</div>
        </div>
        <el-row :gutter="8">
            <el-col :span="12">
