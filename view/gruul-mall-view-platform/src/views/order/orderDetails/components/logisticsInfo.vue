...
import type { Apipackage } from '@/views/order/types/order'
import type { PropType } from 'vue'
import { SellTypeEnum } from '@/views/good/types'
import type { OrderDetail, ShopOrderItem } from '@/apis/order/type/order'
...
async function initDeliveryPageAll() {
    ...
    const { code, data } = await doGetDeliveryPackage<Apipackage>($props.order.no, $props.order.shopOrders[0].no, shopId as string)
...
const handleTabClick = async () => {
    ...
    console.log($props.order.shopOrders[0]?.shopOrderItems, '$props.order.shopOrders[0]?.shopOrderItems')
    ...
    try {
        const { data, code } = await doGetLogisticsTrajectoryByWaybillNo(
            orderItem.expressCompanyCode,
            orderItem.expressNo,
            undefined,
            orderItem.receiverMobile,
        )
        if (code !== 200) return ElMessage.error('包裹详情获取失败')
        if (!data.data) {
                            ...
                            <template #default="{ row }">
                                <div style="display: flex; align-items: center">
                                    ...
                                    <div>
                                        ...
                                        <div>{{ SellTypeEnum[(row as ShopOrderItem)?.sellType] }}</div>
                                    </div>
                                </div>
                            </template>
                        </el-table-column>
