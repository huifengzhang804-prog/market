    ...
    <div class="table_container">
        ...
        >
            ...
            <el-table-column align="left" label="状态" width="120">
                <template #default="scope">
                    <el-switch v-model="scope.row.userRole.enable" :disabled="scope.row.loading" @change="changeAdminStatus(scope.row)" />
                </template>
            </el-table-column>
        ...
        <el-form ref="formRef" :model="adminPage.form" :rules="adminPage.rules" label-width="80px" style="width: 460px; margin-top: 20px">
            ...
            <template v-else>
                ...
                <el-form-item label="角色" prop="roleId">
                    <el-select v-model="adminPage.form.roleId" clearable :placeholder="rolePlaceholder">
                        <el-option v-for="role in adminPage.roles" :key="role.id" :label="role.name" :value="role.id" :disabled="!role.enabled" />
                    </el-select>
                </el-form-item>
            </template>
        </el-form>
...
const adminPage = reactive<any>({
    ...
    rules: {
        ...
        username: [
            { required: true, message: '用户名需包含两类：大写字母、小写字母、数字、特殊字符', trigger: 'change' },
            {
...
const queryUserByKeywords = async (keywords: string, cb: AutocompleteFetchSuggestionsCallback) => {
    if (!keywords) {
        cb([])
        return
    }
