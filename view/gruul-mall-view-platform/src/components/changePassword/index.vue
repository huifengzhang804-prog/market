...
import { doGetMyResetPasswordSms, doPutMyResetPassword, doPostLogout, doGetCaptchaSlider } from '@/apis/sign'
...
const changePasswordFlag = computed(() => {
    return useAdminInfo().getterChangePasswordFlag
})
...
const info = ref<AccountInfo>(new Storage().getItem('userInfo'))
...
watch(
    () => useAdminInfo().getterChangePasswordFlag,
    (val) => {
        if (val) {
...
const getCaptchaSlider = async () => {
    const { code, data } = await doGetCaptchaSlider(info.value.mobile || '', 'RESET_PASSWORD')
    if (code !== 200) {
        return
    }
    if (data.captcha) {
...
const handleSubmit = async () => {
    ...
    if (!isValida) return
    useAdminInfo().SET_CHANGE_PASSWORD_FLAG(false)
        ...
        >
            ...
            <el-form-item label="验证码" prop="code" style="margin-bottom: 18px">
                <div class="verification_code">
                    ...
                    <el-button class="el-Verification" type="primary" plain :disabled="isCountdown" round @click="sendSmsCode">{{
