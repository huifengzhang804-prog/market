...
const tableData = ref<LinkSelectItem[]>([])
...
const handleSelect = () => {
    const currentItem: LinkSelectItem | undefined = tableData.value.find((item) => item.id === selectId.value)
    if (currentItem) {
        Object.assign(linkSelectItem.value, currentItem)
    }
}
// const sameJudge = (): boolean => {
//     return typeNameMap[linkSelectItem.value.type] && typeNameMap[linkSelectItem.value.type].name === name
// }
// TODO:差商品专区接口
