class Storage {
    ...
    public setItem(key: string, value: any, time: number = 60 * 60 * 24 * 30) {
        if (value === undefined) value = null
        const _key = this.name + key
        const _value = { value, time: time * 1000 + new Date().getTime() }
        localStorage.setItem(_key, JSON.stringify(_value))
    }
    public getItem(key: string) {
        if (this.hasItem(key)) {
            return JSON.parse(`${localStorage.getItem(this.name + key) ? localStorage.getItem(this.name + key) : '{"value":null}'}`).value
        }
        return null
    }
