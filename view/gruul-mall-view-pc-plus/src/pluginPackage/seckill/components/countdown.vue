...
import { FIXED_POINT_TIME, toHHmmss } from '@/pluginPackage/seckill/components/countdown'
...
function dayFormat(time: string) {
  ...
  const rangeDateNum = (new Date(time).getTime() - currentTime.getTime()) / (1000 * 3600)
...
<template>
  <view class="countdown-view">
    <slot name="default" :time-arr="countdownTimeS">
      ...
      <text class="countdown-view--item">
        {{ dayFormat(startTime) || '00' }}
      </text>
