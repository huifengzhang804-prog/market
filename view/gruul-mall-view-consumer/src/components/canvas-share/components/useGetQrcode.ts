...
export interface Options_ {
  path: string
  params: { shopId: Long; goodsId: Long; distributorCode: string }
}
 ...
 * @param {Long} goodId
 */
export const SHARE_HERF = (goodId: Long, shopId: Long) => {
  const env = import.meta.env.VITE_BASE_URL
  return `${env.replace(/api/, 'h5')}#/pluginPackage/pages/commodityInfo/InfoEntrance?goodId=${goodId}&shopId=${shopId}`
}
// 获取二维码图片
export const useGetQrcode = () => {
  ...
  const handleGetQrcode = async (fn: (qrcode: string) => void, callBackfn: () => void = () => {}, options_?: Options_) => {
    ...
    if (useUserStore().userInfo.info.distributorCode) {
      qrcodeData.params.distributorCode = useUserStore().userInfo.info.distributorCode
    }
