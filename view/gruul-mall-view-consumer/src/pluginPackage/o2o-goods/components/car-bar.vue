<template>
  <view>
    ...
    <carPopup
      ...
      @get-price="getPrice"
    />
    <stock-modal v-model="showStockModal" :stock-modal-data="stockModalData" @init="initCarData" />
  </view>
</template>
...
const props = defineProps({
  shopInfo: {
    type: Object as PropType<GetShopInfoRes>,
    default: () => ({}),
  },
  deliveryMoney: {
    type: Number as PropType<Long>,
    default: 0,
  },
})
...
// 激活价格按钮
const payAacive = computed(() => Number(divTenThousand(price.value)) >= Number(props.deliveryMoney) && Number(divTenThousand(price.value)) !== 0)
// 价格
const price = ref(0)
// 获取价格
...
const handleSubmit = async (rule: boolean) => {
  ...
  let isSameDistributionMode = true,
    sampleDistributionMode: DistributionKeyType | undefined = undefined
  for (let i = 0; i < carList?.[0]?.products?.length; i++) {
    const product = carList?.[0]?.products?.[i]
    if (product.distributionMode?.length > 1) {
      isSameDistributionMode = false
      break
    }
    if (sampleDistributionMode && sampleDistributionMode !== product.distributionMode?.[0]) {
      sampleDistributionMode = product.distributionMode?.[0]
      isSameDistributionMode = false
      break
    }
    sampleDistributionMode = product.distributionMode?.[0]
  }
  if (isSameDistributionMode === false) {
    // 多配送方式跳转页面
  ...
  if (sampleDistributionMode) {
    carList[0].distributionMode = sampleDistributionMode
  }
  // 没有多配送方式直接去结算
