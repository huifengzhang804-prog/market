...
import { computed, type PropType, ref, reactive, watch, provide } from 'vue'
...
import { doGetCategoryLevel, doGetCommodityBySecCateId } from '@/apis/decoration/shop'
import type { ApiCategoryData } from '@/basePackage/pages/merchant/types'
import type { DeCategoryType } from '@/pages/platform/types'

import SelectSpec from '@/basePackage/pages/merchant/components/o2o-shop/classification/components/select-spec.vue'
...
async function initCategoryConfig() {
  const data = $props.decorationProperties
...
// 规格弹框组件
const selectSpecRef = ref<InstanceType<typeof SelectSpec>>()
// 打开弹框方法
...
// 传递打开规格弹框方法
provide('selectSpec', openSelectSpec)
</script>
  ...
  <view>
    ...
    <div v-else class="content">
      ...
      <class-three
        ...
        :list="commodityList"
        :shop-info="$props.shopInfo"
        @list-load-more="handleListLoadMore"
        @change-tab="handleChangeNavTab"
      />
    </div>
    <view style="height: 90rpx"> </view>
    <SelectSpec ref="selectSpecRef" :shop-id="$props.shopInfo.id" />
  </view>
</template>
