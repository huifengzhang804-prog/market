...
const getShopInfo = async () => {
  const { code, data } = await doShopInfo('SUPPLIER')
  if (code === 200) {
    supplier.value = data
  }
}
  ...
  <view>
    ...
    <view class="content">
      <view class="content_info">
        ...
        <view>供应商名称：{{ supplier.name }}</view>
        <view>申请时间：{{ supplier.createTime }}</view>
        <view v-if="supplier?.extra?.auditTime">审核时间：{{ supplier?.extra?.auditTime }}</view>
        <view v-if="supplier?.extra?.reasonForRejection !== 'null' && supplier?.extra?.reasonForRejection"
