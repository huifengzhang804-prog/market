...
import coupon from '@/pages/plugin/coupon/components/decorationComponents/coupon.vue'
import { useStatusBar } from '@/hooks'
import { doGetShopPagesDetail } from '@/apis/decoration/shop/index'
...
onLoad((params: any) => {
  ...
  if (params.id) {
    initDecorationFromCus(params.id, params.shopId)
  }
})
 ...
 * 自定义页面获取装修数据
 */
async function initDecorationFromCus(pageId: string, shopId: Long) {
  const { code, data } = await doGetShopPagesDetail(pageId, shopId)
  if (code !== 200) {
    uni.showToast({
      icon: 'none',
      title: '获取装修失败',
...
<template>
  ...
  <scroll-view scroll-y class="scroll" :style="{ height: `${scrollViewHeight}px` }">
    <view v-for="(item, index) in decorateList" :key="index">
      ...
      <shop-good v-if="item.value === 'goods'" :decoration-properties="item.formData" />
      ...
      <coupon v-if="item.value === 'coupon'" :decoration-properties="item.formData" />
    </view>
  </scroll-view>
</template>
