<template>
    <view :class="uFromData.inputAlign == 'right' ? 'flex-end' : ''" class="u-radio-group u-clearfix">
        <slot></slot>
    </view>
</template>
...
/**
 * radioRroup 单选框父组件
 * 单选框用于有一个选择，用户只能选择其中一个的场景。搭配u-radio使用
...
export default {
    ...
    computed: {
        ...
        // 拉取父组件新的变化后的参数
        parentData() {
            return [this.valueCom, this.disabled, this.activeColor, this.size, this.labelDisabled, this.shape, this.iconSize, this.width, this.wrap]
        },
    },
    methods: {
