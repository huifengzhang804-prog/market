    ...
    emits: ["start", "end", "change"],
		...
		methods: {
			checkKeepRunning() {
				...
				let lastTimestamp = Number(storage.get(this.uniqueKey + '_$uCountDownTimestamp'));
				...
				if(this.keepRunning && lastTimestamp && lastTimestamp > nowTimestamp) {
					...
					// 清除本地保存的变量
					storage.remove(this.uniqueKey + '_$uCountDownTimestamp');
					// 开始倒计时
					this.start();
				} else {
			...
			setTimeToStorage() {
				...
				if(this.secNum > 0 && this.secNum <= this.seconds) {
					...
					storage.set( this.uniqueKey + '_$uCountDownTimestamp',nowTimestamp + Number(this.secNum))
				}
			}
		},
