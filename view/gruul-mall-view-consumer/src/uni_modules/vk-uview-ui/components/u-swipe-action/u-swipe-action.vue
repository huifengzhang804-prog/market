	...
	<view class="">
		<movable-area class="u-swipe-action" :style="{ backgroundColor: bgColor }">
			<movable-view class="u-swipe-view" @change="change" @touchend="touchend" @touchstart="touchstart"
				direction="horizontal" :disabled="disabled" :x="moveX" inertia :style="{
	...
	methods: {
		...
		getActionRect() {
			this.$uGetRect('.u-swipe-action').then(res => {
				// res.width 获取节点宽度可能为 0 导致视图错位 这里采用传入的默认值作为代替
				this.movableAreaWidth = res.width ? res.width : this.swipeActionWidth //res.width;
