    ...
    <SchemaForm v-model="searchType" :columns="columns" @searchHandle="search" @handleReset="handleReset">
        <template #otherOperations>
            <router-link to="/inventory/storage/add">
                <el-button type="primary" round style="margin-left: 12px; border-radius: 4px">新增出入库</el-button>
            </router-link>
        </template>
    </SchemaForm>
</template>
...
export type SearchType = Record<'no' | 'stockChangeType' | 'date' | 'operationPhone', string>
