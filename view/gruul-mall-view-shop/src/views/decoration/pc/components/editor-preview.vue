...
import renderComponents from '@/views/decoration/pc/components/render-components.vue'
...
const emit = defineEmits(['change', 'add', 'del', 'update:startComponents'])
...
const hideMessage = (text: string) => {
    ElMessage.success(`${text}组件已隐藏,您可以从 “固定组件--固定组件” 中重新启用。`)
}
</script>

<template>
    <el-scrollbar class="preview" min-height="calc(100vh - 80px)">
        <render-components :components="fixedTopComponents" :dragging="dragging" :highlight-current="false" :fixed="true" :edit="false" />
        <!-- 顶部不可拖动组件 -->
        <render-components
        ...
        <render-components :dragging="dragging" :components="fixedBottomComponents" :highlight-current="false" :fixed="true" :edit="false" />
