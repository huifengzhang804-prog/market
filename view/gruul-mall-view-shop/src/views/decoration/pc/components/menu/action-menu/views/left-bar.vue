...
watch(
    () => props.activeName,
    () => {
        activeNameRef.value = props.activeName
    },
    { immediate: true },
)
...
<template>
    <ul class="menu" :class="{ 'menu-active': enterName }">
        <li
            ...
            :key="item.text"
            class="menu__item"
            :class="{ active: item.text === activeNameRef }"
            :index="item.text"
        ...
        >
            <div>
                <q-icon svg :name="item.text === enterName || item.text === activeNameRef ? item.activeIcon : item.icon" size="36px" />
