...
const $emit = defineEmits(['close', 'update:sku', 'update:show'])
            ...
            <el-row justify="space-between">
                <el-form-item label="限购类型">
                    <el-radio-group v-model="currentRadioChoose">
                        ...
                        <el-radio value="PRODUCT_LIMITED">商品限购</el-radio>
                        <el-radio v-if="$props.sku.length > 1" label="SKU_LIMITED">规格限购</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item v-if="currentRadioChoose === 'SKU_LIMITED' && $props.sku.length !== 1" label="批量设置">
                    <el-input-number style="width: 100px" :controls="false" :precision="0" :min="1" :max="9999999" @change="handleBatch" />
                </el-form-item>
            </el-row>
            <el-form-item v-if="currentRadioChoose === 'PRODUCT_LIMITED'" label="数量">
                <el-input-number v-model="commodityNum" :controls="false" :precision="0" style="width: 90vw" :min="1" :max="9999999" />
            </el-form-item>
            <el-form-item v-else-if="currentRadioChoose === 'SKU_LIMITED' && $props.sku.length !== 1">
                <el-table :data="$props.sku" height="50vh">
                    ...
                    <el-table-column label="限购数量" align="center">
                        <template #default="{ row }">
                            <el-input-number v-model="row.limitNum" :controls="false" :min="1" :precision="0" :max="9999999" />
                        </template>
                    </el-table-column>
                </el-table>
            </el-form-item>
