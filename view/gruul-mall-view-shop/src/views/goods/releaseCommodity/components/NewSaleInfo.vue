...
const realPriceValidatePass = (rule: any, value: any, callback: any) => {
    if (value <= 0) {
        ...
        callback(new Error('划线价应大于等于销售价'))
    } else {
        callback()
    }
}
...
const updateChildName = (newVal: string, oldVal: string, rowIndex: number, tagIndex: number) => {
...
async function dataDisplay() {
    if ($route.query.id) {
        ...
        skuData.skus.forEach((item: any) => {
            if ($route?.query?.isCopy) item.id = void 0
            item.initStock = 0
...
<template>
    <el-form ref="currentFormRef" :model="submitForm" :rules="formRules" label-width="100px">
        ...
        <div v-if="radio === 'MUTI_SPEC'">
            <GoodNorms :class-arr="submitForm.specGroups" @change-class="changeClassHandle" @update-child-name="updateChildName" />
            <GoodNormTable
            ...
            <el-row :gutter="8">
                ...
                <el-col :span="12">
                    <el-form-item label="库存类型">
                        <el-select v-model="submitForm.skus[0].stockType" :disabled="submitForm.productType === 'REAL_PRODUCT'">
                ...
                <el-col v-if="!submitForm.skus[0]?.id" :span="12">
                    <el-form-item label="库存" prop="skus[0].initStock">
                        <div class="inputnumber">
                            <el-input-number
                                v-model="submitForm.skus[0].initStock"
                                :disabled="submitForm.skus[0].id || submitForm.skus[0].stockType === 'UNLIMITED'"
                                :max="999999"
                                :min="0"
                                :precision="0"
                                :controls="false"
