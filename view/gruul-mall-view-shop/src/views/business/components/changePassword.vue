...
import { doGetUserDataAccount, doGetMyResetPasswordSms, doPutResetPassword, doGetResetPasswordSms, doPostLogout } from '@/apis/sign'
...
const handleSendVerificationCode = async () => {
    ...
    const { code, data } = await doGetResetPasswordSms(passwordForm.value.mobile)
    if (code !== 200) {
        ElMessage.error('验证码获取失败')
        return
    }
    passwordForm.value.code = data
