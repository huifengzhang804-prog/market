...
import { MessageAndShopAdmin } from '@/views/message/customerService/types'
    ...
    >
        <div class="product-box">
            ...
            <div class="product-info">
                ...
                <span v-if="product.h5">{{ product.price?.estimate ? `¥${divTenThousand(product.price.estimate).toFixed(2)}起` : '' }}</span>

                <span v-else>￥{{ String(product.price?.estimate).split('~')[0] }}起</span>
            </div>
        </div>
        <div class="foot">
            <p @click="previewVisible = true">查看商品</p>
        </div>
        <el-dialog v-model="previewVisible" title="商品详情" center width="900px" destroy-on-close top="3vh">
            <CommodityDetails v-if="product.id" :commodityId="product.id.split(':')[1] || product.id" />
        </el-dialog>
    </div>
</template>
