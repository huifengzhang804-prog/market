import { OrderDiscountItem, ORDERSTATUS, PACKAGESTATUS, SHOPITEMSTATUS, ShopOrderItem, SHOPORDERSTATUS } from '@views/order/types/order'
import { AFSSTATUS } from '@views/afs/types'
import Decimal from 'decimal.js'
import { Order, OrderDiscount } from '@/apis/order/types'
import { IcStatus, IcStatusText } from '@/views/afs/types/api'
import { OrderListType } from '@/views/message/components/types'
...
export function calculate(order: Order, showIc: boolean = true): OrderMergedItems {
        ...
        (counterMap, item) => {
            ...
            if (!counter) {
                counter = {
                    ...
                    statusBuilder: StatusBuilder.create(orderStatus as keyof typeof ORDERSTATUS, shopOrderStatus as keyof typeof SHOPORDERSTATUS),
                    sellType: '',
                }
    ...
    const { receiveTime, deliverTime } = shopOrder.extra || {}
    //统计所有的状态的商品数量
    ...
    if (commented > 0 && commented >= ok) {
        ...
        orderMergedItems.state.activeStep = 1
        // 订单商品总数 = 售后成功商品数 + 已发货数
        // 如果 已发货数 + 售后成功商品数 === shopOrderItems.length，则订单状态为 待收货(OrderStateStatus.UN_RECEIVE)
    }
    if (ok === 0) {
...
function itemDiscountMap(orderDiscounts: OrderDiscount[], shopOrderItems: ShopOrderItem[]) {
    ...
    const itemKeyDiscountMap = orderDiscounts.reduce((map, current) => {
        ...
        items.forEach((item: OrderDiscountItem) => {
...
export const filterMainOrderStatus = (orderInfo: OrderListType) => {
