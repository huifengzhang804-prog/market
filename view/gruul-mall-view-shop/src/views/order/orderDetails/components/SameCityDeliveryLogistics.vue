...
import CourierIcon from '@/assets/images/ic/courier.png'
import UserIcon from '@/assets/images/ic/user.png'
...
const OrderParam = computed(() => {
    return { shopId: orderShopInfo.value.id, orderNo: props.order.no }
})
/**
 * 获取同城配送订单详情
 */
...
const toRefresh = async () => {
    ...
    //如果订单已送达,异常 则不刷新
    if (['ERROR', 'DELIVERED'].includes(detail?.orders?.[0]?.status)) {
        return
    }
