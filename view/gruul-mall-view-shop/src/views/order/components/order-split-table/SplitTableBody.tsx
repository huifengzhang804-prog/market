...
export default defineComponent({
    ...
    setup(prop, { slots, emit }) {
        ...
        const packageMapToDom = (row: ApiOrder, index: number): any[] => {
            const tds: any[] = []
            const packageMap = row.shopOrders?.[0].packageMap || new Map()
        ...
        const packageMapToHeaderRowDom = (row: ApiOrder, index: number): any[] => {
            const tds: any[] = []
            const packageMap = row.shopOrders?.[0].packageMap || new Map()
            packageMap.forEach((items, key) => {
                tds.push()
                                    ...
                                    <tr>
                                        <td colspan={prop.columns.length}>
                                            <div class={['body--header', prop.rowHeaderClass, { close: row.close }]}>
                                                ...
                                                {slots.header({ row, shopOrderItems: row?.shopOrders?.[0]?.shopOrderItems || [] })}
                                            </div>
                                        </td>
                                    </tr>
                                )}
