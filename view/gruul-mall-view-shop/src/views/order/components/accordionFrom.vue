...
const HandleSearch = () => {
    const { no, buyerNickname, productName, receiverName, distributionMode, platform } = SearchFromData
    const params = {
        no,
        buyerNickname,
        productName,
...
const showDistributeModeSearchCondition = computed(() => $route.name !== 'orderIndex')
// 表单配置项
const columns = [
    {
        label: '商品名称',
    ...
    },
    {
        ...
        hideInSearch: showDistributeModeSearchCondition.value,
        fieldProps: {
            placeholder: '请选择配送方式',
        },
    },
...
const exportData = (e: MouseEvent) => {
    $emit('exportData', SearchFromData, e)
}
</script>

<template>
    <SchemaForm v-model="SearchFromData" :columns="columns" @searchHandle="HandleSearch" @handleReset="handleReset">
        <template #otherOperations>
            <el-button id="export_btn" type="primary" round @click="exportData">导出</el-button>
        </template>
    </SchemaForm>
</template>
