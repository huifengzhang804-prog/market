...
import Cropper from '@/components/q-upload/cropper.vue'
...
const { currentFormModel, getMaterialCategoryList } = useMaterialCategoryList()
...
const handleSearch = (searchType: SearchTypes) => {
    ...
    Object.keys(searchCondition).forEach((key) => {
        searchCondition[key as keyof SearchTypes] = paramsValue[key as keyof SearchTypes]
    })
...
function handleCrossDomainImage(img: string) {
    const isDev = import.meta.env.VITE_NODE_ENV === 'development'
    if (isDev) {
        img = img.replace(import.meta.env.VITE_OSS_URL, `${currentUrl.split('/shop/#')[0]}/cosimg`)
    }
    console.log(img)
    return img
}
...
const useTailorFn = () => {
    ...
    const res = handleCrossDomainImage(selectLists.value?.[0]?.url)
    cropperSrc.value = res
...
const useCheckedMaterialFn = () => {
    ...
    for (let i = 0; i < selectLists.value.length; i++) {
        ...
        CheckedMaterialUrlLists.value.push(selectLists.value?.[i]?.url)
        name = selectLists.value?.[i].name
    }
    emit('checkedFileLists', CheckedMaterialUrlLists.value, name)
    CheckedMaterialUrlLists.value = []
    handleClose()
}
...
<template>
    ...
    >
        <CategorySearch v-model:categoryId="searchCondition.categoryId" :flex="false" @onSearchParams="handleSearch" />
        <div class="uploadingBox">
            ...
            <el-tooltip content="支持素材格式（jpg、gif、jpeg、png、webp、bmp、MP4），每次限10个文件，单个文件 10MB 以内" placement="top">
                <QIcon name="icon-wenhao1" color="#000" size="20px" />
            </el-tooltip>
        </div>
