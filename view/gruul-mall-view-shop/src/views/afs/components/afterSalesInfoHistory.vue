...
import { getAfsOrderStatusCn, getAfsStatusCn, aRefundWhy, systemInfo, Desc } from '@/composables/useAfsStatus'
import { doPutAfsRequestAuditByAfsNo, doGetAfssHistory, doGetUserDataByBuyerId, doPutAfsconfirmDeliveryRefunds } from '@/apis/afs'
import type { AFSSTATUS } from '@/views/afs/types/'
import type { ApiBuyersData, ApiShopInfo, ProcessingHistory, ApiHistory } from '@/views/afs/types/api'
...
function initTimeout(data: ApiHistory) {
    ...
    if (data.status === 'RETURN_REFUND_REQUEST') {
        const createOrderTime = new Date(data.overdueTime).getTime()
        timeout.value = createOrderTime
    } else {
 ...
 */
const sellType = ref('')
function initShopInfo(res: ApiHistory) {
...
/**
 * 初始化协商历史（协商历史中添加金额和状态） 同意请求超时  确认返回超时   返回超时
 */
...
const initProcessingHistory = () => {
    ...
    processingHistory.value = processingHistory.value.map((item) => {
        ...
        if (isConsumer) {
            ...
            return { ...item, name: systemInfo.name, logo: systemInfo.avatar, afsStatusCn: afsInfo.desc, isConsumer: true, type: afsInfo.type }
        }
 ...
 * 同意退款
 */
const handleDetermine = async (type: string | undefined) => {
...
async function AfsRequestAuditByAfsNo(type: keyof typeof AFSSTATUS, agree: boolean, remark: string, receiver?: string) {
    ...
    if (code !== 200) return ElMessage.error(msg || '审核失败')
    ...
    <div class="afterSalesInfo">
        <div class="afterSalesInfo__status">
            <div class="afterSalesInfo__status--title">
                ...
                <countdown v-if="afsConfigObj?.isCountdown" :unmounted="afsConfigObj?.isCountdown" :timeout="timeout" />
            </div>
            <div v-if="afsConfigObj?.msg" class="afterSalesInfo__status--prompt">{{ afsConfigObj?.msg }}</div>
            <el-row v-if="afsConfigObj?.isShowAgreebtn && sellType !== 'CONSIGNMENT'">
                <el-button
                    type="primary"
                    :disabled="!(currentAfsNo && currentAfsStatus)"
                    round
                    @click="handleDetermine(afsConfigObj?.agreebtnText)"
                    >{{ afsConfigObj?.agreebtnText }}</el-button
                >
                <el-button :disabled="!(currentAfsNo && currentAfsStatus)" round @click="dialogVisible = true"
                    >拒绝{{ afsConfigObj?.type }}</el-button
                >
            </el-row>
            <el-row v-if="sellType === 'CONSIGNMENT'" class="red-bold">代销商品售后由供应商处理，店铺无需操作</el-row>
        </div>
        <el-row :gutter="8">
            <el-col :span="12">
                ...
                <el-row :gutter="8" style="line-height: 1.5; margin-left: 10px">
                    ...
                    <!-- <el-col :span="12">
                        <el-link type="primary">联系买家</el-link>
                    </el-col> -->
                        ...
                        <el-row :gutter="8">
                            ...
                            <template v-else>
                                ...
                                <el-col v-if="historyItem?.historyBuyerReturnedInfo?.expressRefund?.expressCompany?.expressCompanyName" :span="6"
                                    >物流公司：{{ historyItem.historyBuyerReturnedInfo.expressRefund.expressCompany.expressCompanyName }}</el-col
                                >
                                <el-col v-if="historyItem?.historyBuyerReturnedInfo?.expressRefund?.expressCompany?.expressNo" :span="6"
                                    >物流单号：{{ historyItem.historyBuyerReturnedInfo.expressRefund.expressCompany.expressNo }}</el-col
                                >
                                ...
                                <el-col v-if="historyItem.reason" :span="8"
                                    >退款原因：{{ historyItem.reason && aRefundWhy[historyItem.reason].title }}</el-col
                                >
                                ...
                                <div style="display: flex">
                                    <el-image
                                        ...
                                        :src="
                                            evidence
                                                ? evidence
                                                : 'https://gimg2.baidu.com/image_search/src=http%3A%2F%2Fhbimg.huabanimg.com%2F129ac6e8f658c6d73ef688b4b3265f4bcc1e3c1a4b538-pHD1zx_fw658&refer=http%3A%2F%2Fhbimg.huabanimg.com&app=2002&size=f9999,10000&q=a80&n=0&g=0n&fmt=auto?sec=1663835058&t=bc879dcf32e7dcd65fbdc92df8bfd251'
                                        "
                                        :preview-src-list="[
                                            evidence
                                                ? evidence
        ...
        <el-dialog v-model="dialogVisible" width="30%" @close="refuseToReason = ''">
            ...
            <el-input v-model="refuseToReason" height="100px" type="textarea" :rows="4" placeholder="请输入拒绝原因" max="140" maxlength="140">
            </el-input>
            <template #footer>
